...
export const CATEGORIES: Record<string, Category> = {
  ...
  biz_transfer:  { id: 'biz_transfer',  name: '이체',          emoji: '↔️', color: '#94A3B8', scope: 'business' },
  biz_owner_draw:{ id: 'biz_owner_draw',name: '사장 인출',     emoji: '🚪', color: '#6B7684', scope: 'business' },
  ...
  biz_capital:     { id: 'biz_capital',     name: '자본 투입',  emoji: '💼', color: '#3182F6', kind: 'income', scope: 'business' },

  // Personal income for owner — when business pays the owner
  owner_pay:    { id: 'owner_pay',    name: '사장 보수',     emoji: '💵', color: '#3182F6', kind: 'income', scope: 'personal' },
};
...
export const isTransferCategory = (catId: string) =>
  catId === 'transfer' || catId === 'biz_transfer' || catId === 'biz_owner_draw' || catId === 'biz_capital' || catId === 'owner_pay';
...
const MERCHANT_HINTS: Array<[RegExp, string]> = [
  ...
  [/치킨|피자|마라탕|김밥|버거|맥도날드|롯데리아|버거킹|kfc|서브웨이|식당|국밥|돈까스|치즈|푸드|만두|냉면|쌀국수|식사|레스토랑|배달|배민|요기요|쿠팡이츠/i, 'food'],
  [/지하철|버스|택시|kakao\s?t|타다|gs25|cu|cu편의점|편의점|gs칼텍스|sk엔크린|주유/i, 'transit'],
  [/cgv|메가박스|롯데시네마|영화|뮤지컬|콘서트|스파|찜질방|문화|netflix|넷플릭스|유튜브|youtube|spotify|디즈니/i, 'culture'],
  [/올리브영|화장품|미용실|네일|헤어|뷰티|피부과|에스테틱/i, 'beauty'],
  [/약국|병원|의원|치과|한의원|건강|영양제|보험/i, 'health'],
  [/무신사|29cm|쿠팡|네이버쇼핑|11번가|지마켓|옥션|이마트|홈플러스|롯데마트|다이소|이케아/i, 'shopping'],
