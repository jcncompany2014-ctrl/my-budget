    ...
    <Sheet open onClose={onCancel}>
      ...
      <Field label="월 기본급">
        <input
          ...
          onChange={(ev) => setDraft({ ...draft, baseSalary: Number(ev.target.value) || 0 })}
          placeholder="0"
        ...
        <p className="mt-1" style={{ color: 'var(--color-text-3)', fontSize: 'var(--text-xxs)' }}>
          예상 월 지출: <span className="tnum">{fmt(Math.round(draft.baseSalary * 1.1))}원</span>{' '}
          (4대보험 약 +10%)
        </p>
      </Field>
      <Field label="입사일">
        <input
          ...
          onChange={(ev) => setDraft({ ...draft, startDate: ev.target.value })}
...
function Btn({
  ...
  const styles = {
    primary: {
      bg: disabled ? 'var(--color-gray-200)' : 'var(--color-primary)',
      color: disabled ? 'var(--color-text-4)' : '#fff',
    },
