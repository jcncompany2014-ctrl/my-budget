...
const COLORS = ['#3182F6', '#00B956', '#F472B6', '#FF8A1F', '#8B5CF6', '#14B8A6', '#FFCC00', '#EF4444'];
    ...
    <>
      ...
      {editing && (
        <Editor
          ...
          isNew={creating}
          onSave={(v) => {
            if (creating) add(v); else update(v.id, v);
...
function Editor({ v, isNew, onSave, onDelete, onCancel }: {
  v: Vendor; isNew: boolean;
  onSave: (v: Vendor) => void; onDelete?: () => void; onCancel: () => void;
}) {
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-end justify-center bg-black/40">
      ...
      >
        ...
        <div className="mb-3">
          <label className="mb-1.5 block" style={{ color: 'var(--color-text-2)', fontSize: 'var(--text-sm)', fontWeight: 600 }}>구분</label>
        ...
        <div className="mb-3">
          <label className="mb-1.5 block" style={{ color: 'var(--color-text-2)', fontSize: 'var(--text-sm)', fontWeight: 600 }}>연락처</label>
          <input
            value={draft.contact ?? ''}
