...
export default function MonthlyReportPage() {
  ...
  const data = useMemo(() => {
    ...
    monthTx
      .filter(isExpense)
      .forEach((t) => byCat.set(t.cat, (byCat.get(t.cat) ?? 0) + Math.abs(t.amount)));
  ...
  return (
    <>
      <TopBar title={`${year}년 ${month + 1}월 리포트`} />
      ...
      <section className="px-5 pb-3 pt-1 print:hidden">
        ...
        >
          ...
          <div className="relative">
            ...
            >
              {data.income - data.expense >= 0 ? '+' : '−'}
              {fmt(data.income - data.expense)}원
            </p>
            {data.income + data.expense > 0 && (
              <div
                className="mt-3 flex h-[4px] overflow-hidden rounded-full"
            ...
            <div className="mt-3 grid grid-cols-2 gap-2 text-white">
              <div>
                <p
                  style={{ opacity: 0.85, fontSize: 10, fontWeight: 700, letterSpacing: '0.02em' }}
                >
                  {mode === 'business' ? '매출' : '수입'}
                </p>
